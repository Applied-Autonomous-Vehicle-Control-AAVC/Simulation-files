...
# Initial conditions
x_v, y_v, psi_v = 0.0, 0.0, np.radians(45.0)  # initial position and heading
...
plt.subplot(211)
plt.plot(time, np.degrees(psi_p), ':r', time, np.degrees(psi), 'k', linewidth=1.5)
plt.legend([r'$\psi_{ref}$', r'$\psi$'], fontsize=font_size)
plt.ylabel(r'$\psi$ ($^\circ$)', fontsize=font_size)

plt.subplot(212)
plt.plot(time, tau_ac, 'k', linewidth=1.5)
plt.xlabel('Time (s)', fontsize=font_size)
plt.ylabel(r'Control Input', fontsize=font_size)

plt.tight_layout()
plt.savefig('yaw_control.eps', format='eps')
plt.show()
...
axs[0, 0].plot(time, DCPA[:, 0], 'k', linewidth=1.5)
axs[0, 0].set_xlim([0, Sim_time])
axs[0, 0].set_ylabel(r'$DCPA$ (m)', fontsize=font_size)

axs[0, 1].plot(time, Distance_ob[:, 0], 'k', linewidth=1.5)
axs[0, 1].set_xlim([0, Sim_time])
axs[0, 1].set_ylim([0, 2000])
axs[0, 1].set_ylabel(r'$R$ (m)', fontsize=font_size)

axs[1, 0].plot(time, TCPA[:, 0], 'k', linewidth=1.5)
axs[1, 0].set_xlim([0, Sim_time])
axs[1, 0].set_xlabel('Time (s)', fontsize=font_size)
axs[1, 0].set_ylabel(r'$TCPA$ (s)', fontsize=font_size)

axs[1, 1].plot(time, Risk[:, 0], 'k', linewidth=1.5)
axs[1, 1].set_xlim([0, Sim_time])
axs[1, 1].set_ylim([0, 1])
axs[1, 1].set_xlabel('Time (s)', fontsize=font_size)
axs[1, 1].set_ylabel(r'$Risk$', fontsize=font_size)

plt.tight_layout()
plt.savefig('CPAplots.eps', format='eps')
plt.show()
